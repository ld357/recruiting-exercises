...
The problem is compute the best way an order can be shipped (called order) given inventory across a set of inventory (called inventory distribution). 
...
The second input will be a list of object with warehouse name and inventory amounts (inventory distribution) for these items. For example the inventory across two inventory called owd and dm for apples, bananas and oranges could look like
...
You can assume that the list of inventory is pre-sorted based on cost. The first warehouse will be less expensive to ship from than the second warehouse. 
...
*Should split an item across inventory if that is the only way to completely ship an item:*
